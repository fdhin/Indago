...
| WU005 | `WUComponentHealth` | WindowsUpdate | System | DISM health check, CBS.log corruption analysis (8 HRESULTs), last SFC result extraction, and component store sizing via /AnalyzeComponentStore |
| WU006 | `WUEventTimeline` | WindowsUpdate | System | WU event log deep dive: chronological timeline from WindowsUpdateClient, BITS-Client, and System logs (SCM crash events for WU services), HRESULT extraction and translation via 25-entry map with scriptlet routing |
| WU007 | `WUEnvironmentAudit` | WindowsUpdate | System | WU agent version (from wuaueng.dll with registry fallback), OS edition/build/UBR with feature update eligibility table (Win10 2004-22H2, Win11 21H2-26H1), .NET Framework version decode, PowerShell version, third-party AV detection with 10-vendor WU-interference flagging, update management tool inventory (SCCM, Intune with Update Ring policy detection via PolicyManager, PatchMyPC/WSUS/ManageEngine/Automox) with co-management detection, and Visual C++ prerequisite check |
| WU008 | `WUDatastoreRepair` | WindowsUpdate | System | REMEDIATION: DataStore.edb integrity check (size, freshness, ESE transaction log accumulation) with conditional auto-rebuild (stop wuauserv+BITS, rename to .bak, clear ESE logs, restart). Full COM re-registration of 17 DLLs (wuaueng, wuapi, wups, wups2, wuwebv, wucltux, wudriver, atl, urlmon, mshtml, msxml, msxml3, msxml6, jscript, scrrun, shdocvw, softpub). Advisory SDDL audit comparing current service security descriptors to Windows defaults for wuauserv/BITS/CryptSvc (reports but does not overwrite, provides sc.exe sdset command if critical SYSTEM/BA ACEs are missing). BITS job cleanup via Get-BitsTransfer -AllUsers (cancels Error/TransientError, stale suspended >7 days). Stale pending.xml rename to .bak if >7 days old. Pre-state JSON export |
| WU009 | `WUServiceReset` | WindowsUpdate | System | **Remediation**: Stops 5 WU services (UsoSvc, wuauserv, BITS, CryptSvc, AppIDSvc), renames SoftwareDistribution and catroot2 caches with timestamped .bak suffix, re-registers 8 core COM DLLs, resets Winsock catalog, restarts services, and verifies post-remediation state. Pre-state captured to Indago Logs. Reports locking process PID with taskkill command on rename failure. |
...
| DEF006 | `DEFPlatformVersion` | DefenderEndpoint | System | Platform (MoCAMP), engine, and NIS engine version validation against hardcoded baselines with N-2 deprecation awareness; update channel/ring decode across GPO/MDM/Local; platform update event log queries with HRESULT translation; orphaned MoCAMP update lock detection; WSUS version pinning correlation |
| DEF007 | `DEFEventAnalysis` | DefenderEndpoint | System | Defender event log timeline: scan events (start/complete/cancel/fail), definition updates, configuration changes (Event 5007), service health heartbeats, threat detection and remediation with ThreatStatusID/AdditionalActionsBitMask decode, SecurityCenter state change and WMI sync errors, WHC log (if available), and 10-entry HRESULT translation map with scriptlet routing |
| DEF008 | `DEFRemediation` | DefenderEndpoint | System | **Remediation**: 4-mode action script (Auto/UpdateOnly/CleanupOnly/Full). Forces signature update via source cascade (WU, MMPC, WSUS, FileShares), removes ghost third-party AV SecurityCenter2 registrations (orphan-only safety guard), removes legacy DisableAntiSpyware/DisableAntiVirus/ForceDefenderPassiveMode overrides (domain-join aware), re-enables RTP (Tamper Protection aware), restarts WinDefend/WdNisSvc. Full mode adds MpCmdRun cache clear, dangerous exclusion removal, and quick scan validation. Pre-state captured to Indago Logs. |
...
| BL005 | `BLEscrowCheck` | BitLocker | System | Recovery key escrow pipeline diagnostics: FVE escrow gate detection, AAD device identity health (zombie PRT detection), escrow event history with HRESULT translation, cloud endpoint connectivity, key protector escrow confirmation, and WinRE status |
| BL006 | `BLPolicyConflict` | BitLocker | System | GPO vs MDM conflict detection: FVE registry enumeration with cipher/TPM decode, MDM PolicyManager settings with XML parsing, side-by-side conflict comparison (encryption method, startup PIN, AD backup deadlock, PIN length), MDMWinsOverGP assessment with BitLocker CSP limitation warning, and orphaned GPO settings detection via domain-join state |
| BL007 | `BLEventAnalysis` | BitLocker | System | BitLocker event timeline: encryption lifecycle from Management log (start/complete/rollback/suspension, key protectors, TPM seal, escrow, silent encryption failure, WinRE/TPM blockers), Operational log API errors, System log TPM events (dictionary attack lockout), DrivePreparationTool log (if available), and 11-entry HRESULT translation map with scriptlet routing |
| BL008 | `BLReadinessCheck` | BitLocker | System | Encryption readiness dry run: service dependency gate (BDESVC/TBS/RpcSs), volume state via manage-bde -status (conversion status, ghost state, orphaned protectors), WMI readiness via Win32_EncryptableVolume (GetConversionStatus/GetProtectionStatus), BCD integrity via bcdedit (bootmgr/OS loader path validation), WinRE health via reagentc /info (null-GUID detection, CVE-2024-20666 context), PCR validation via manage-bde -protectors -get (ideal 7,11 vs degraded 0,2,4,11 OROM fallback), and single go/no-go readiness verdict |
| BL009 | `BLTpmRemediation` | BitLocker | System | REMEDIATION: 3-mode TPM and key protector remediation (Auto/DecryptFirst/ClearTPM). Stale RecoveryPassword protector cleanup via destructive refresh, ghost suspended state handling (FullyEncrypted+ProtectionOff with missing TPM protector injection), TPM re-initialization via Initialize-Tpm, TPM clear with OEM physical presence warnings (Lenovo/Dell/HP), BitLocker suspension for boot lockout prevention, new RecoveryPassword generation, Entra ID escrow via BackupToAAD-BitLockerKeyProtector with dsregcmd /status pre-check (AAD/Hybrid join + PRT validation), AD DS fallback for on-prem, Event ID 845 confirmation, NTE_BAD_KEYSET (0x80090016) motherboard swap detection, service dependency validation (bdesvc/tbs/KeyIso), pre-state JSON export |
...
| FW005 | `FWRuleDiagnostic` | Firewall | System | Rule corruption and bloat diagnostics: total rule count (CIM with registry fallback), per-profile distribution, 6-field duplicate detection, orphaned application path validation (including %LocalAppData% expansion), registry store size vs 14 MB MpsSvc limit, enabled/disabled ratio, and Event ID 4953 rule corruption detection |
